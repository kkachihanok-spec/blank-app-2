...
# 3. 커스텀 CSS (합격 점수 컬러만 #808ce5로 변경)
st.markdown("""
    <style>
    .stApp {
    ...
    /* 🔥 [수정] 합격 점수 컬러를 #808ce5로 변경 */
    .score-text-fail { color: #AF40FF !important; -webkit-text-fill-color: #AF40FF !important; background: none !important; }
    .score-text-pass { color: #808ce5 !important; -webkit-text-fill-color: #808ce5 !important; background: none !important; }
...
if st.session_state.analyzed_data:
    ...
    for i, config in enumerate(quiz_configs):
        ...
        ans = st.radio(f"R_{q_key}", st.session_state[q_key], index=None, key=f"ans_f_v11_{q_key}", label_visibility="collapsed")
