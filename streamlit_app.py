...
if st.session_state.analyzed_data:
    ...
    if all_answered:
        ...
        # --- 커스텀 폴딩 가이드 섹션 (번역 카드 톤앤매너와 일치) ---
        st.divider()
        
        # 번역 섹션과 유사한 공통 스타일 변수
        theme_bg = "rgba(45, 53, 72, 0.25)"  # 번역 카드 배경색
        theme_accent = "#4a5fcc"            # 번역 카드 테두리 및 메인 포인트 블루
        ...
        st.markdown(f"""
            <style>
                .custom-details {{
                    ...
                    border-left: 4px solid {theme_accent}; /* 번역 카드와 동일한 테두리 포인트 */
                ...
                .custom-summary {{
                    padding: 24px; font-size: 1.3rem; font-weight: 800; color: #FFFFFF !important; /* 제목 화이트 고정 */
                    cursor: pointer; list-style: none; display: flex; justify-content: space-between; align-items: center;
                }}
                .custom-summary:hover {{ background: rgba(74, 95, 204, 0.05); }}
                .custom-summary::after {{ 
                    content: '▼'; color: {theme_accent}; font-size: 0.9rem; transition: transform 0.3s; 
                }}
                .custom-details[open] .custom-summary::after {{ transform: rotate(180deg); }}
                .guide-content {{ padding: 0 24px 24px 24px; animation: fadeIn 0.5s ease; }}
                @keyframes fadeIn {{ from {{ opacity: 0; transform: translateY(-10px); }} to {{ opacity: 1; transform: translateY(0); }} }}
                
                .guide-link-card-custom {{
                    background: rgba(255,255,255,0.03); padding: 14px; border-radius: 8px; 
                    text-align: center; color: #8b92b2 !important; border: 1px solid rgba(255,255,255,0.05);
            ...
            <details class="custom-details" open>
                ...
                <div class="guide-content">
                    <h4 style="color: {theme_accent}; margin-top: 10px; font-weight: 800;">📝 학습 가이드</h4>
                    <p style="color: #FFFFFF; line-height: 1.8; font-size: 1.15rem; margin-bottom: 25px;">{advice}</p>
                    <hr style="border-color: rgba(255,255,255,0.1); margin-bottom: 25px;">
                    <h4 style="color: {theme_accent}; font-weight: 800; margin-bottom: 15px;">🔗 추천 학습 레퍼런스</h4>
