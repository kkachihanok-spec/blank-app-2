...
if st.session_state.analyzed:
    lyrics_input = st.session_state.lyrics_text
    st.divider()
    ...
    if not df_all.empty:
        ...
        st.subheader(f"Q. 이 가사에서 가장 많이 사용된 단어는 '{top_word}'입니다. 이 단어의 품사는 무엇일까요?")
        
        # 퀴즈 선택지
        options = ["명사", "동사", "형용사", "부사"]
        user_choice = st.radio("정답을 골라보세요!", options, index=None, key="quiz_radio")
        
        if user_choice:
            if user_choice == top_pos:
                st.success(f"정답입니다! 🎉 '{top_word}'은(는) 이 가사에서 핵심적인 역할을 하는 **{top_pos}**입니다.")
                st.balloons()
            else:
                st.error(f"아쉬워요! 다시 한번 위쪽의 '분석 데이터' 표를 확인해볼까요? 🧐")
